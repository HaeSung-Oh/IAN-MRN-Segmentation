...
class MultiScalePooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, pool_sizes=(1, 2, 3)):
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(out_channels * len(pool_sizes), out_channels, kernel_size=1, bias=False),
            norm_layer(out_channels),
            nn.ReLU(inplace=True)
        )
...
# ---------- MSCPConv2d  Class ----------
class MSCPConv2d (nn.Module):
    def __init__(self, in_channels, channels, kernel_size, stride=(1, 1), padding=(0, 0),
                 dilation=(1, 1), groups=1, bias=True,
                 radix=2, reduction_factor=4,
                 rectify=False, rectify_avg=False, norm_layer=None,
                 dropblock_prob=0.0, **kwargs):
        super(MSCPConv2d , self).__init__()
        ...
        self.pool_extract = MultiScalePooling(
    ...
    def forward(self, x):
        ...
        x_splits = torch.split(x, rchannel // self.radix, dim=1)

        processed_splits = []
        for i, split in enumerate(x_splits):
            if i == 0:
                processed_splits.append(self.c1(split))
            elif i == 1:
                processed_splits.append(self.c2(split))
            elif i == 2:
                processed_splits.append(self.c3(split))
            elif i == 3:
                processed_splits.append(self.c4(split))

        out = sum(processed_splits)
...
class ObsNet_Remove_att_r_v7(nn.Module):
